...
struct WsCreatePick3 {

    class Request: WsRequest {
        typealias Response = WsCreatePick3.Response
        
        var pickSources: [WsCreatePick3.PickSource]?
        ...
        func merge(json: JSON?) {
            ...
            if let pickSourcesArrayValue = json["pickSources"].array {
                ...
                for item in pickSourcesArrayValue {
                    if let value = WsCreatePick3.PickSource(json: item), var pickSources = pickSources {
...
class Response: JSONSerializable {
    var code: WsCreatePick3.Code?
    var pickCreateResults: [WsCreatePick3.PickCreateResult]?
...
func merge(json: JSON?) {
    ...
    if let codeStringValue = json["code"].string {
        if let code = WsCreatePick3.Code(rawValue: codeStringValue) {
            self.code = code
        }
    }
    ...
    if let pickCreateResultsArrayValue = json["pickCreateResults"].array {
        ...
        for item in pickCreateResultsArrayValue {
            if let value = WsCreatePick3.PickCreateResult(json: item), var pickCreateResults = pickCreateResults {
...
enum Code: String {
    ...
    case INVALID_SOURCE
    case MULTIPLE_ORDERS_FOUND
    case FAILED
}

class PickCreateResult: JSONSerializable {
    var result: WsCreatePick3.Result?
    var orderLineId: String?
...
func merge(json: JSON?) {
    ...
    if let resultStringValue = json["result"].string {
        if let result = WsCreatePick3.Result(rawValue: resultStringValue) {
            self.result = result
        }
    }
