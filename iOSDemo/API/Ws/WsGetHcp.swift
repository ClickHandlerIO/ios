...
struct WsGetHcp {
    ...
    class Request: WsRequest {
        typealias Response = WsGetHcp.Response
...
class Response: JSONSerializable {
    var code: WsGetHcp.Code?
    var detail: WsGetHcp.HcpDetail?
...
func merge(json: JSON?) {
    ...
    if let codeStringValue = json["code"].string {
        if let code = WsGetHcp.Code(rawValue: codeStringValue) {
            self.code = code
        }
    }
    
    self.detail = WsGetHcp.HcpDetail(json: json["detail"])
}
...
class HcpDetail: JSONSerializable {
    var id: String?
    var orgId: String?
    var orgType: OrgType?
    var orgName: String?
